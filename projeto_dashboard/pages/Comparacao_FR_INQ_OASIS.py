...
def plot_comparacao_por_imagem_valence_arousal(df_comparado, jogadora_id, nome_jogadora):
    ...
    fig = px.bar(
        ...
        barmode="group",
        height=500,
        title=f"Comparação de Valence e Arousal por Imagem - {nome_jogadora}"
    )
    fig.update_layout(
...
# ✅ Seletor por nome
atletas = df_comparado[["ID", "Jogadora"]].drop_duplicates().sort_values("Jogadora")
nome_sel = st.selectbox("Seleciona uma atleta:", atletas["Jogadora"])
id_sel = atletas[atletas["Jogadora"] == nome_sel]["ID"].values[0]

df_sel = df_comparado[df_comparado["ID"] == id_sel].copy()

st.subheader(f"📊 Comparação Integrada Valence e Arousal - {nome_sel}")
plot_comparacao_por_imagem_valence_arousal(df_comparado, id_sel, nome_sel)

# Dispersões
st.subheader(f"📌 Dispersão FaceReader vs OASIS - {nome_sel}")
corr_val_fo = df_sel[["Valence", "Valence_OASIS"]].corr().iloc[0, 1]
corr_ar_fo = df_sel[["Arousal", "Arousal_OASIS"]].corr().iloc[0, 1]

fig_val_fo = px.scatter(df_sel, x="Valence_OASIS", y="Valence", text="Imagem",
    title=f"Valence: OASIS vs FaceReader ({nome_sel})")
...
fig_val_fo.update_layout(xaxis=dict(linecolor="black", range=[-1, 1]), yaxis=dict(linecolor="black", range=[-1, 1]))
st.plotly_chart(fig_val_fo, use_container_width=True)

fig_ar_fo = px.scatter(df_sel, x="Arousal_OASIS", y="Arousal", text="Imagem",
    title=f"Arousal: OASIS vs FaceReader ({nome_sel})")
...
fig_ar_fo.update_layout(xaxis=dict(linecolor="black", range=[0, 1]), yaxis=dict(linecolor="black", range=[0, 1]))
st.plotly_chart(fig_ar_fo, use_container_width=True)

# Dispersão Inquérito vs OASIS
st.subheader(f"📌 Dispersão Inquérito vs OASIS - {nome_sel}")
...
fig_val_io = px.scatter(df_sel, x="Valence_OASIS", y="Valence_Inquerito", text="Imagem",
    title=f"Valence: OASIS vs Inquérito ({nome_sel})")
fig_val_io.update_traces(marker=dict(size=10, color="purple"), textposition="top center", textfont=dict(color="black"))
fig_val_io.update_layout(xaxis=dict(linecolor="black", range=[-1, 1]), yaxis=dict(linecolor="black", range=[-1, 1]))
st.plotly_chart(fig_val_io, use_container_width=True)

fig_ar_io = px.scatter(df_sel, x="Arousal_OASIS", y="Arousal_Inquerito", text="Imagem",
    title=f"Arousal: OASIS vs Inquérito ({nome_sel})")
fig_ar_io.update_traces(marker=dict(size=10, color="darkorange"), textposition="top center", textfont=dict(color="black"))
fig_ar_io.update_layout(xaxis=dict(linecolor="black", range=[0, 1]), yaxis=dict(linecolor="black", range=[0, 1]))
